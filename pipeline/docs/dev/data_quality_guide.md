...
BETTERJOBS_DB.INTERNAL.A_REVIEW_SALARY_QUEUE             -- Salary items flagged for review
```
...
FROM BETTERJOBS_DB.INTERNAL.A_REVIEW_SALARY_QUEUE;
...
SELECT * FROM BETTERJOBS_DB.INTERNAL.A_REVIEW_SALARY_QUEUE
...
WHERE BRIDGE_ID IN (
    SELECT BRIDGE_ID
    FROM BETTERJOBS_DB.INTERNAL.A_REVIEW_SALARY_QUEUE zsrq
