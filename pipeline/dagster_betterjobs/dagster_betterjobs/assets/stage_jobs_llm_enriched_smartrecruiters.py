"""
SmartRecruiters Platform LLM Enrichment Asset

Platform-specific LLM enrichment for SmartRecruiters jobs using shared processing logic.
Part of ENHANCEMENT-031: Company-based partitioned LLM enrichment for parallel processing.

This asset processes SmartRecruiters jobs partitioned by company name (A-Z, 0-9, other)
...
@asset(
    ...
    description="Extract structured information from SmartRecruiters job descriptions using Gemini LLM (partitioned by company)"
)
def stage_jobs_llm_enriched_smartrecruiters(context: AssetExecutionContext, config: PartitionedLLMEnrichmentConfig) -> Dict[str, Any]:
    """
    Enrich SmartRecruiters job data with AI-extracted information using Gemini LLM.

    This asset processes jobs from the SmartRecruiters platform partitioned by company name
    ...
    context.log.info(f"🚀 [SMARTRECRUITERS] Starting partitioned LLM enrichment for companies: {partition_key}")
    ...
    metadata = create_platform_metadata(stats, "smartrecruiters")
    metadata["partition_key"] = MetadataValue.text(partition_key)
    context.add_output_metadata(metadata)

    context.log.info(f"✅ [SMARTRECRUITERS] Partitioned LLM enrichment completed successfully for {partition_key}")
