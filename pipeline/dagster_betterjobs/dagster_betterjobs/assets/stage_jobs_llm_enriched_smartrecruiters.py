...
This asset processes only SmartRecruiters jobs for AI-powered information extraction,
...
class LLMEnrichmentConfig(Config):
    ...
    batch_size: int = 15  # Standard batch size for SmartRecruiters jobs
...
@asset(
    ...
    description="Extract structured information from SmartRecruiters job descriptions using Gemini LLM"
)
def stage_jobs_llm_enriched_smartrecruiters(context: AssetExecutionContext, config: LLMEnrichmentConfig) -> Dict[str, Any]:
    """
    Enrich SmartRecruiters job data with AI-extracted information using Gemini LLM.

    This asset processes jobs from the SmartRecruiters platform and extracts
    ...
    context.log.info("🚀 [SMARTRECRUITERS] Starting platform-specific LLM enrichment...")
    ...
    context.log.info(f"✅ [SMARTRECRUITERS] LLM enrichment completed successfully")
