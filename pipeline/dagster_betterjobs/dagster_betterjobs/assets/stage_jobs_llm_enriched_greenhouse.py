...
This asset processes only Greenhouse jobs for AI-powered information extraction,
...
class LLMEnrichmentConfig(Config):
    ...
    batch_size: int = 20  # Larger batches for simpler Greenhouse jobs
...
@asset(
    ...
    description="Extract structured information from Greenhouse job descriptions using Gemini LLM"
)
def stage_jobs_llm_enriched_greenhouse(context: AssetExecutionContext, config: LLMEnrichmentConfig) -> Dict[str, Any]:
    """
    Enrich Greenhouse job data with AI-extracted information using Gemini LLM.

    This asset processes jobs from the Greenhouse platform and extracts
    ...
    context.log.info("🚀 [GREENHOUSE] Starting platform-specific LLM enrichment...")
    ...
    context.log.info(f"✅ [GREENHOUSE] LLM enrichment completed successfully")
